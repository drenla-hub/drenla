                ...
                <table role="presentation" width="560" cellpadding="0" cellspacing="0" style="background-color:#ffffff; border:1px solid #e4dfd6;">
                    ...
                    <tr>
                        <td style="padding: 32px 40px;">
                            <p style="margin:0 0 8px; font-size:11px; font-weight:700; letter-spacing:0.2em; text-transform:uppercase; color:#8a7f6c;">{{ ucfirst($document->type) }} &middot; {{ $document->reference_number }}</p>
                            <h1 style="margin:0 0 16px; font-size:22px; font-weight:400; color:#161219;">{{ $document->currency }} {{ number_format((float) $document->total_amount, 2) }}</h1>
                            <p style="margin:0 0 24px; font-size:14px; line-height:1.6; color:#3a3327;">
                                Your {{ $document->type }} is attached as a PDF@if($document->due_date), due {{ $document->due_date->format('d M Y') }}@endif. You can also view it, along with the rest of your engagement, in your Drenla client portal.
                            </p>
