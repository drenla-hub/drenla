...
<div class="space-y-8">
    ...
    <div class="flex items-end justify-between border-b border-[#1a1a1a] pb-8">
        <div>
            ...
            <h1 class="mt-3 text-[32px] font-light tracking-[-0.025em] text-white">Focus Areas</h1>
            <p class="mt-2 text-[13px] leading-relaxed text-[#555]">Public capability and positioning content served through the API.</p>
        </div>
        <a href="{{ route('admin.focus-areas.create') }}"
            ...
            <tbody class="divide-y divide-[#0e0e0e]">
                @forelse ($focusAreas as $focusArea)
                    @php
                        ...
                        $sColor = $sMap[$focusArea->status] ?? '#555';
                    @endphp
                    <tr class="transition-colors hover:bg-[#080808]">
                        <td class="px-5 py-4">
                            <p class="text-[13px] font-medium text-white">{{ $focusArea->title }}</p>
                            <p class="mt-0.5 text-[11px] text-[#444]">{{ $focusArea->slug }}</p>
                        </td>
                        <td class="px-5 py-4">
                            <span class="text-[10px] font-bold uppercase tracking-[0.15em]"
                                  style="color: {{ $sColor }}">{{ $focusArea->status }}</span>
                        </td>
                        <td class="px-5 py-4 text-[12px] text-[#666]">{{ $focusArea->sort_order ?? '—' }}</td>
                        <td class="px-5 py-4 text-[12px] text-[#666]">{{ optional($focusArea->published_at)->format('d M Y') ?: '—' }}</td>
                        <td class="px-5 py-4">
                            @if ($focusArea->featured)
                        ...
                        <td class="px-5 py-4">
                            <div class="flex items-center justify-end gap-5">
                                <a href="{{ route('admin.focus-areas.edit', $focusArea) }}"
                ...
                @empty
                    <tr><td colspan="6" class="px-5 py-12 text-center text-[12px] text-[#444]">No focus areas yet.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
